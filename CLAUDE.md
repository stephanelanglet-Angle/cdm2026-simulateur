...
- Bracket KO (matchs 73-104) : la sim les **projette**. Carte « Groupes & tableau final » de Castrol (`renderKO`/`koTab`) les **affiche** désormais (lecture seule) : `renderGroupStandings` (12 classements live, ordre exact Art.13 via `fifaRankGroup` quand le groupe est terminé, sinon tri pts/diff/buts) + `renderBracket` = **projection CONTINUE** : `bracketData` classe chaque poule avec son ordre **provisoire** (pas besoin qu'elle soit finie) → remplit le R32, puis projette les **vainqueurs KO par probabilité du modèle** (`koWin(h,a)=grid().pH+pD/2`, nul → tirs au but 50/50) jusqu'au champion, avec le **% affiché** sur chaque cellule (favori en vert). Style : **gras** = qualifié confirmé (poule terminée pour 1er/2e ; toutes les poules finies pour les 3es), **italique** = provisoire. **Saisie KO opérationnelle (Castrol)** : filtre « 🏆 Phase finale » (liste) + **clic sur une cellule du bracket** → saisie résultat + pronos par joueur (comptés au classement via `standings`), **toggle qualifié t.a.b.** pour les nuls, **vainqueur propagé** au tour suivant. `koState()` résout équipes + vainqueurs (réels si `KO.res` saisi, sinon projetés `koWin`) + confirmation `cA/cB/cW` (un 8e+ n'est saisissable que si ses 2 équipes sont réelles). État `KO={res,win,preds}` sérialisé + synchronisé (schéma v2 ; `findMatch`/`koMatchObj`). **Régie** (`renderKObracket`) : carte « Tableau final » = vrai bracket R32 **embarqué** (`KO_R32_NAMES`, calculé une fois des vrais résultats Art.13, mappé aux indices `TEAMS` par normalisation accents/apostrophes) ; **clic = verrouille le vainqueur** d'un match (`state.ko['r32-0']=idx`…), `koReal()` résout le bracket déterministe (un match n'est cliquable que si ses 2 équipes sont réelles), `koSimulate()` = **Monte-Carlo conditionné** (réutilise `simKO`, 5000 sims) → **proba de titre live** par équipe. Persisté localStorage + sync room 7x (`state.ko`). ⚠️ Les 2 KO sont **séparés** (Castrol = scores+pronos scorés ; régie = vainqueur pour conditionner la sim).
